...
let g:std_io_run_commands = {'cpp': "'c++ -Wall -Wextra -Werror --std=c++17 -g -fsanitize=address -o ' . expand('%:p:r') . ' ' . expand('%:p') . ' && ' . expand('%:p:r')", 'java': "'javac ' . expand('%:p') . ' && java ' . expand('%:r')", 'python': "'python ' . expand('%:p')" }
